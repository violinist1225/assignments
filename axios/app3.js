...
const imgUrl = document.theform.imgUrl
...
function addTodo(e) {
    ...
    /* sends the new todo (which we made on lines 16-19) to the server using the post api call*/.then(() => renderItems()) /* this is triggered if the api post was successful - basically: if the API was successful THEN call this annonymous arrow function, which in turn calls our renderItems function. it passes in an argument which is an object containing info about the posting from the server, but we are not using it in our function so I omitted it.   */.catch(error => console.log(error))
    /* this is triggered if the api post failes. it receives info about the reason for the failure, which we log to the console */
}

function renderItems() { // This function is grabbing each item in array of objects and creating a list to show on DOM with properties.
...
function createItem(itemObj) {
    ...
    const h3 = document.createElement('h3') // creates p element
    const h3Span = document.createElement('span')
    h3Span.textContent = itemObj.description
    h3Span.style.color = "purple"
    h3Span.style.fontSize = "25px"


    const checkbox = document.createElement('input') // creates checkbox witih input datatype
    h3.className = itemObj.completed ? "done" : "not-done" // creates class name in Javascript for p tag and assigns it to tertiary boolean. If item status is complted, then assign done or not done class on DOM.
    ...
    checkbox.addEventListener("change", (e) => {
        ...
        // .Put updates server with new content. Takes 2 arguments: 1st is the url we want to change (specifically), 2nd: What to change content to? In this case, completed key -part of object on server is set to false.  Then .then method called. Which calls renderItems function (that updates the DOM).then((response) => {
            .then(response => {
                renderItems()
            })
        .catch((error) => {
    ...
    h3.textContent = itemObj.title
    h3.style.fontSize = "30px"
    h3Span.style.alignItems = "center"
    ...
    editButton.addEventListener("click", e => {
        ...
        // hides p tag (newP created earlier). Since changing CSS Property, have to use Style Property in dot notation!
        // STOPPED HERE FOR EDIT BUTTON
    })
    deleteButton.addEventListener("click", (e) => {
        removeId(itemObj._id)
    })
    
    newLi.append(checkbox, h3, h3Span, deleteButton) // all elements can be added on one line, in ORDER of appeara
    if (itemObj.imgUrl) {
        const picture = document.createElement("img")
        picture.src = itemObj.imgUrl
        newLi.append(picture)
    }
    if(itemObj.price){
        const price = document.createElement("h2")  
        const priceSpan = document.createElement('span')
        priceSpan.textContent = itemObj.price
        priceSpan.style.color = "orange"
        newLi.append(priceSpan)
    }
...
function removeId(id) {
    ...
    // any of the axios methods return a PROMISE, so then must use .then and .catch methods after each promise..then
    .then(reponse => {  
    renderItems();
    })
    .catch(error => console.log(error))
